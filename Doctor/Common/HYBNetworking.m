...
//    if (url) {
//        url = [NSString stringWithFormat:@"/api%@",url];
//    }
...
/////////////////////
//  if ([manager.baseURL.absoluteString isEqualToString:@"http://www.test.sghuitingche.com"]) {
//    url = [NSString stringWithFormat:@"/api%@",url];
//  }
...
//        error.code = responseCode.statusCode;
      if ([error code] < 0 && sg_cacheGet) {// 获取缓存
        ...
        if (response) {
          ...
          [self handleCallbackWithError:error fail:fail withStutusCode:responseCode.statusCode];
...
+ (HYBURLSessionTask *)downloadWithUrl:(NSString *)url
  ...
  session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
    ...
    if (error == nil) {
      ...
      [self handleCallbackWithError:error fail:failure withStutusCode:responseCode.statusCode];
...
+ (void)handleCallbackWithError:(NSError *)error fail:(HYBResponseFail)fail withStutusCode:(NSInteger)statusCode{
  if ([error code] == NSURLErrorCancelled) {
