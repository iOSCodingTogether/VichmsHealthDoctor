...
- (void)doStartByCaller
{
    ...
    [[NIMAVChatSDK sharedSDK].netCallManager start:callees type:wself.callInfo.callType option:option completion:^(NSError *error, UInt64 callID) {
        if (!error && wself) {
            ...
            if (error) {
                NSLog(@" 连接失败： %@",error);
                [MBProgressHUD showAlertWithView:self.view andTitle:@"连接失败"];
        ...
        if (!error) {
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    if (wself.chatRoom.count == 1) {
                        [MBProgressHUD showAlertWithView:self.view andTitle:@"通话失败"];
            ...
            if (room && !room.count && !_userHangup) {
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    ...
                    if (wself && room && room.count == 1) {
                        ...
                        [MBProgressHUD showAlertWithView:self.view andTitle:@"暂时无人接听"];
...
//                                                          position:CSToastPositionCenter];
        ...
        case NIMNetCallControlTypeBusyLine: {
            ...
            [MBProgressHUD showAlertWithView:self.view andTitle:@"对不起,当前主播正忙"];
...
- (void)onResponsedByOther:(UInt64)callID
    ...
    [MBProgressHUD showAlertWithView:self.view andTitle:@"已在其他端处理"];
...
- (void)onHangup:(UInt64)callID
    ...
    if (self.callInfo.callID == callID) {
        ...
        if (self.callInfo.localRecording) {
            ...
            [MBProgressHUD showAlertWithView:self.view andTitle:@"当前通话结束"];
...
- (void)onNTESTimerFired:(NTESTimerHolder *)holder{
    if (holder == self.diskCheckTimer) {
        ...
        if (!_calleeResponsed) {
            [MBProgressHUD showAlertWithView:self.view andTitle:@"接听超时"];
