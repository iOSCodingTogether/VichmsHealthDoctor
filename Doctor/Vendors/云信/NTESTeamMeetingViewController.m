...
//                    DDLogError(@"join meeting error! error %@",error);
        ...
        {
            NSLog(@"reserve meeting error! error %@",error);
            dispatch_async(dispatch_get_main_queue(), ^{
                onError;
            });
        }
...
- (void)joinMeeting
{
    ...
    [[NIMAVChatSDK sharedSDK].netCallManager joinMeeting:self.meeting completion:^(NIMNetCallMeeting * _Nonnull meeting, NSError * _Nonnull error) {
        ...
        {
            NSLog(@"join meeting error! error %@",error);
            dispatch_async(dispatch_get_main_queue(), ^{
                onError;
            });
        }
...
- (void)onMeetingError:(NSError *)error
               meeting:(NIMNetCallMeeting *)meeting
{
    NSLog(@"on meeting error! error %@",error);
...
- (void)onControl:(UInt64)callID from:(NSString *)user type:(NIMNetCallControlType)control
{
    ...
    {
        NSLog(@"on receive control command %zd from %@",control,user);
...
- (void)checkServiceEnable:(void(^)(BOOL))result{
    ...
    if (audioStatus == AVAuthorizationStatusRestricted
        ...
        UIAlertController *alert=[UIAlertController alertControllerWithTitle:@"麦克风权限受限,无法聊天" message:@"如还想使用此功能，请在手机设置中打开照相机"  preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *act2=[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        }];
        [alert addAction:act2];
        [self presentViewController:alert animated:YES completion:nil];
